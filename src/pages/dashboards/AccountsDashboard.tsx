...
export function AccountsDashboard() {
  ...
  useEffect(() => {
    // Quick head request to check if accounts.html has been placed in the public/reports folder
    fetch("/reports/accounts.html", { method: "HEAD" })
      .then((res) => {
        if (res.ok) {
          setHasReport(true);
        } else {
        ...
        <div className="flex-1 flex items-center justify-center container-wide relative z-10 py-16">
          <div className="w-full max-w-xl text-center flex flex-col items-center">
            ...
            <p className="text-text-muted text-[0.95rem] leading-relaxed mb-6 max-w-md">
              The trading performance dashboard for the Accounts department will be available once the monthly report is uploaded. Please contact your administrator.
            </p>
